...
public interface ArticleRepository extends JpaRepository<Article, Long> {
	...
	@Query(value = "SELECT * FROM article WHERE " + DEFAULTPARAMS + " ORDER BY date DESC", nativeQuery = true)
	List<Article> findAllValidArticles();

	// Obtém os artigos mais visualizados decrescente.
	@Query(value = "SELECT * FROM article WHERE " + DEFAULTPARAMS
	...
	@Query(value = "SELECT * FROM article WHERE " + DEFAULTPARAMS + " AND id = :id", nativeQuery = true)
	...
	@Query(value = "SELECT CASE WHEN COUNT(id) > 0 THEN true ELSE false END FROM article WHERE " + DEFAULTPARAMS
			+ " AND id = :id", nativeQuery = true)
	boolean existsById(@Param("id") Long id);

	// Atualiza a quantidade de views do artigo pelo Id.
	@Modifying
	@Query(value = "UPDATE article SET views = views + 1 WHERE " + DEFAULTPARAMS + " AND id = :id", nativeQuery = true)
